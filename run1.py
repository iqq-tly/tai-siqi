...
class PI_DeepONet(nn.Module):
    ...
    def loss_bcs(self,u1,u2, u3,x, t, output):
        ...
        # Compute loss
        loss = torch.mean((output.flatten() - s_pred1) ** 2)
        return loss
    ...
    def train(self,u1,u2,u3,dataloader1,dataloader2):
        ...
        self.optimizer = torch.optim.LBFGS(params, lr=0.01,history_size=20, line_search_fn="strong_wolfe",
...
model1 =KAN([3, 3, 1], base_activation=nn.Identity)
model2 = KAN([3,3, 1], base_activation=nn.Identity)
model3 = KAN([3,3, 1], base_activation=nn.Identity)
model4 = KAN([200, 3,3], base_activation=nn.Identity)
model5 = KAN([2,3,3], base_activation=nn.Identity)
