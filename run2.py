...
class PI_DeepONet(nn.Module):
    ...
    def brunk_net(self,u_1,u_2,u_3,u_s1,u_s2,u_s3):
        u_s1=u_s1.unsqueeze(1)
        u11= torch.cat((u1,u_s1), dim=1)
        u_s2=u_s2.unsqueeze(1)
        u22= torch.cat((u2,u_s2), dim=1)
        u_s3=u_s3.unsqueeze(1)
        u33= torch.cat((u3,u_s3), dim=1)
        BC11=self.model1(u11)
        BC12=self.model2(u11)
        BC13=self.model3(u11)
        BC21=self.model1(u22)
        BC22=self.model2(u22)
        BC23=self.model3(u22)
        BC31=self.mode11(u33)
        BC32=self.model2(u33)
        BC33=self.model3(u33)
        # BC3 = self.model3(u3)
        # print(type(BC1))
        # B=BC1*BC2
        B= torch.cat((BC11,BC12,BC13,BC21,BC22,BC23,BC31,BC32,BC33,), dim=0)
        # loss=torch.mean((BC1.flatten() -u_s1) ** 2+(BC2.flatten() -u_s2) ** 2+(BC3.flatten() -u_s3) ** 2)
        return B

    # Define DeepONet architecture
    def operator_net(self,u_1,u_2,u_3,u_s1,u_s2,u_s3,x,t):
        n=len(x)
        B1=self.brunk_net(u_1,u_2,u_3,u_s1,u_s2,u_s3)
    ...
    # Define PDE residual
    def residual_net(self,u_1,u_2,u_3,u_s1,u_s2,u_s3,x,t):
        s=self.operator_net(u_1,u_2,u_3,u_s1,u_s2,u_s3,x,t)
        s_x =jacrev(self.operator_net,argnums=6)(u_1,u_2,u_3,u_s1,u_s2,u_s3,x,t).sum(dim=0).to(device)
        s_xx =(hessian(self.operator_net,argnums=6)(u_1,u_2,u_3,u_s1,u_s2,u_s3,x,t).sum(dim=0)).sum(dim=0).to(device)
        s_t =jacrev(self.operator_net,argnums=7)(u_1,u_2,u_3,u_s1,u_s2,u_s3,x,t).sum(dim=0).to(device)
    ...
    def loss_bcs(self,u_1,u_2,u_3,u_s1,u_s2,u_s3,x,t, output):
        # Compute forward pass
        s_pred= self.operator_net(u_1,u_2,u_3,u_s1,u_s2,u_s3,x,t)
    ...
    def loss_res(self,u_1,u_2,u_3,u_s1,u_s2,u_s3,x,t,output):
        # Compute forward pass
        pred = self.residual_net(u_1,u_2,u_3,u_s1,u_s2,u_s3,x,t)
        loss = torch.mean((output.flatten() - pred) ** 2)
        return loss



    def train(self,u_1,u_2,u_3,u_s1,u_s2,u_s3,dataloader1,dataloader2):
        ...
        for _ in pbar:
            ...
            for (x_i, t_i,outputs_i),(x_b, t_b, outputs_b) in zip(dataloader1, dataloader2):
                def closure():
                    ...
                    bc_loss= self.loss_bcs(u_1,u_2,u_3,u_s1,u_s2,u_s3,x_i, t_i,outputs_i)
                    pde_loss=self.loss_res(u_1,u_2,u_3,u_s1,u_s2,u_s3,x_b,t_b,outputs_b)
                    # _,brunk_net_loss= model.brunk_net(u_1,u_2,u_3,u_s1,u_s2,u_s3)
...
def generate_one_training_data(key,P,Q,K,M,r,v,T):
    ...
    # x_bc4= random.uniform(subkeys[7], shape=(P // 3, 1), minval=0, maxval3* K)
...
v=0.165856529
T=1
 u_1,u_2,u_3,u_s1,u_s2,u_s3,x_i, t_i,outputs_i, x_b, t_b, outputs_b ,\
         s_bcs_min_value, s_bcs_max_value,x_bcs_min_value, x_bcs_max_value,t_bcs_min_value, t_bcs_max_value\
...
model1 =KAN([3,2,2,2,1], base_activation=nn.Identity)
model2 = KAN([3,2,2,2,1], base_activation=nn.Identity)
model3 = KAN([2,2,2,1], base_activation=nn.Identity)
model4 = KAN([1200,2,2], base_activation=nn.Identity)
model5 = KAN([2,2,2], base_activation=nn.Identity)
model= PI_DeepONet(model1,model2,model4,model5)
model.to(device)
model.train(u_1,u_2,u_3,u_s1, u_s2,u_s3,dataloader1,dataloader2)
