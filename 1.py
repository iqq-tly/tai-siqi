...
def generate_one_training_data(key,M,r,T,v,m,P,Q,K):
    ...
    u=v_fn(xx)
    idx = random.randint(subkeys[1], (P//4,2), 0, max(M,M))
    ...
    np_K=K*(np.ones((P // 4, 1)))

    x_bc1 = np.zeros((P // 4, 1))
    x_bc2 = 100*K*(np.ones((P // 4, 1)))
    x_bc3 = random.uniform(subkeys[2], shape=(P // 4, 1), minval=0, maxval= x_max)
    x_bcs = np.vstack([x_bc1, x_bc2, x_bc3,x_bc4])
    x_bcs= min_max_normalize(x_bcs,0,x_max)

    t_bc1 = random.uniform(subkeys[3], shape=(P // 4, 1), minval=0, maxval=T)
    t_bc2 = random.uniform(subkeys[3], shape=(P // 4, 1), minval=0, maxval=T)
    t_bc3 = np.zeros((P // 4, 1))
    ...
    x_r = random.uniform(subkeys[4], shape=(Q, 1), minval=0, maxval= x_max)
    t_r = random.uniform(subkeys[5], shape=(Q, 1), minval=0, maxval=T)
    x_r = min_max_normalize(x_r,0,x_max)
...
P = 400  # number of output sensors, 100 for each side
...
#train
    ...
    # Initialize model
    branch_layers = [m,100,50]
    trunk_layers = [2,100,50]

    model = PI_DeepONet(branch_layers, trunk_layers)
    model.train(bcs_dataset, res_dataset, nIter=50000)
